...
const Home: React.FC<{ isDarkMode: boolean; toggleTheme: () => void }> = ({ isDarkMode, toggleTheme }) => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#18181B] text-zinc-900 dark:text-[#FFFFFF] selection:bg-zinc-900 selection:text-white dark:selection:bg-white dark:selection:text-zinc-900 font-sans relative transition-colors duration-300">
      ...
      <nav className="absolute top-0 left-0 w-full flex justify-between items-center px-4 md:px-16 py-5 md:py-6 z-50 pointer-events-none">
        <a
      ...
      <main className="w-full">
        ...
        <section className="relative w-full h-[55vh] md:h-screen flex flex-col justify-end overflow-hidden transition-all duration-300">
          ...
          <div id="hero-text-container" className="absolute bottom-0 left-0 z-10 w-full px-[20px] md:px-16 pb-4 md:pb-10 text-center select-none pointer-events-none flex justify-center">
            <h1
        ...
        <section className="w-full px-4 md:px-16 mt-4 md:mt-10 mb-12 md:mb-20 max-w-screen-2xl mx-auto">
          {projects.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              <div className={`w-full h-[396.5px] bg-zinc-100 dark:bg-[#27272A] rounded-lg md:rounded-2xl overflow-hidden hover:scale-[1.02] transition-transform duration-500 ${projects.length === 1 ? 'md:col-span-2' : ''}`}>
              ...
              {projects.length >= 2 && (
                <div className="w-full h-[396.5px] bg-zinc-100 dark:bg-[#27272A] rounded-lg md:rounded-2xl overflow-hidden hover:scale-[1.02] transition-transform duration-500">
        ...
        <section className="px-4 md:px-16 py-24 md:py-32 max-w-screen-2xl mx-auto">
        ...
        <section className="px-4 md:px-16 py-20 max-w-screen-2xl mx-auto">
        ...
        {/* Experience Section */}
        <section className="px-4 md:px-16 py-32 bg-white dark:bg-[#18181B] transition-colors duration-300">
          <div className="max-w-screen-2xl mx-auto">
            <h2 className="text-5xl md:text-7xl font-bold mb-24 tracking-tighter">Experiences</h2>
            <div className="flex flex-col">
              {EXPERIENCES.map((exp, index) => (
                <div key={index} className="group flex flex-col border-t border-zinc-200 dark:border-[#3f3f46] py-12 transition-colors">
                  {/* Header Row */}
                  <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-8">
                    <h3 className="text-2xl md:text-3xl font-semibold text-zinc-900 dark:text-[#FFFFFF] mb-2 md:mb-0">
                      {exp.role}, <span className="italic font-normal">{exp.company}</span>
                    </h3>
                    <span className="text-zinc-500 dark:text-[#A1A1AA] font-secondary text-lg whitespace-nowrap">{exp.year}</span>
                  </div>

                  {/* Description List */}
                  <ul className="list-disc pl-5 space-y-3">
                    {exp.description && exp.description.map((point, i) => (
                      <li key={i} className="text-lg text-zinc-600 dark:text-[#A1A1AA] leading-relaxed pl-2">
                        {point}
                      </li>
                    ))}
                  </ul>
